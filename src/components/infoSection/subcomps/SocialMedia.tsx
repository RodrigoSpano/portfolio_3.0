...
const SocialMedia: React.FC = () => {
  ...
  return (
    <div className='flex text-2xl text-[#1f1c19] dark:text-[#e7d7c6] opacity-75 items-center w-[100px]'>
      <div className='flex gap-5'>
      {
        mediaLinks.map((el: ILinks) => <motion.a whileHover={{scale: 1.2}} key={Math.random()} target='_blank' href={el.link}>{el.icon}</motion.a>)
      }
      </div>
      <Tooltip content={language === 'en' ? 'Download Resume':'Descargar curriculum'} className='dark:bg-[#e7d7c6] opacity-75 text-[#1f1c19] '>
        <Button size='sm' className='text-[#1f1c19] dark:text-[#e7d7c6] text-2xl bg-transparent border-none outline-none '>
