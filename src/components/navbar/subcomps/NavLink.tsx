...
const NavLink = ({title, active, value, toggleActive}: {value:string, title: string, active: boolean, toggleActive:(by:string)=>void}) => {
  ...
  return (
    <Link onClick={() => toggleActive(value)} onMouseEnter={() => !line && setLine(true)} onMouseLeave={() => line && setLine(false)} smooth={true} duration={300} to='' className={`${active && 'dark:text-white translate-x-4'} dark:hover:text-white flex items-center gap-2 cursor-pointer transition-all ease-in duration-300`}>
